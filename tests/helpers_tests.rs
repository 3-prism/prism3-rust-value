...
pub(crate) fn decode_value_wire_value(
    input: serde_json::Value,
) -> Result<ValueWireV1, serde_json::Error> {
...
pub(crate) fn decode_value_wire_payload_str(
    input: &str,
...
pub(crate) fn decode_value_wire_payload_value(
    ...
    decode_value_wire_payload_str(
        &serde_json::to_string(&input).expect("test JSON value should serialize"),
    )
}
